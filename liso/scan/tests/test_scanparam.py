...
from liso.scan.scan_param import JitterParam, ScanParam
...
class TestScanParam(unittest.TestCase):
    def testScanParam(self):
        ...
        self.assertListEqual([-1.0, -1.0, 0.0, 0.0, 1.0, 1.0],
                             param.generate(repeats=2, cycles=1))
        self.assertListEqual([-1.0, -1.0, 0.0, 0.0, 1.0, 1.0] * 3,
    ...
    def testJitterParam(self):
        ...
        self.assertLess(abs(0.1 - np.std(param.generate(repeats=3, cycles=200))), 0.01)

        # test negative sigma
        param = JitterParam('param', 10., sigma=-0.1)
        self.assertLess(abs(1. - np.std(param.generate(repeats=3, cycles=200))), 0.1)
